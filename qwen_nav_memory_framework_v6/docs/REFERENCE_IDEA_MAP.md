...
# v0.6 GaP-lite idea map

The Graph-as-Policy idea is used only as a lightweight engineering pattern.

```text
Full GaP idea:
  policy as directed computation/execution graph
  typed skills
...
v0.6 adaptation:
  no generated policy graph
  no graph interpreter
  no self-learning loop
  yes candidate_ref typed references
  yes validation checkpoints
  yes nav_skill_cards
  yes Feedback.md logs
```

The spatial memory graph remains the main research object. GaP-lite only constrains how Qwen reads memory candidates and how backend commits actions/memory operations.

## Mapped features

```text
Typed dependency / ref selection
  -> memory.candidate_refs and selected_candidate_ref

Skill catalog
  -> memory.nav_skill_cards and prompt rules

Checkpoint / validation
  -> validation_checkpoints and validation_feedback

Recovery edge
  -> invalid go candidate_ref becomes request_observation
  -> invalid revisit memory_op is dropped and duplicate node remains

Feedback loop
  -> save_run() creates Feedback.md for offline analysis
```

## Why this is minimal

v0.6 does not let the VLM write or rewrite the execution graph. The online action space remains the original schema: go, rotate, stop, request_observation. This preserves the slow VLM / fast navigation-skill split while adding stronger runtime validation.
