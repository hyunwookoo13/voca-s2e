...
Candidate reference / GaP-lite policy:
- Use memory.candidate_refs.exits[*].candidate_ref when action=go. Put the chosen value in selected_candidate_ref.
- Do not invent candidate_ref, node_id, or edge_id. Select only refs present in this input snapshot.
- Never go to a candidate_ref whose avoid=true. Prefer request_observation when uncertain.
- memory.nav_skill_cards is the rulebook for when go, request_observation, and confirm_revisit_node are allowed.
- Backend validation_checkpoints will reject unknown refs, avoid=true refs, and spatially implausible memory writes.
...
- Also inspect each candidate's spatial_plausibility. Confirm same_place only when the graph-composed baseline is plausible for parallax/viewpoint change.
- Reject or request more observation when spatial_plausibility.accepted=false, even if the images look similar; this usually indicates visual aliasing from a different distant place.
...
- If a candidate is the same place, add memory_ops with its candidate_ref: [{"op":"confirm_revisit_node","candidate_ref":"revisit_001","confidence":0.0-1.0,"reason":"short evidence"}].
- If the current provisional node and a candidate should be merged, add memory_ops with candidate_ref: [{"op":"request_merge_nodes","candidate_ref":"revisit_001","confidence":0.0-1.0,"reason":"..."}]. The backend will verify and will soft-link/preserve nodes before committing; it will not hard-delete revisit nodes by default.
...
- For action=go, set selected_candidate_ref to one value from memory.candidate_refs.exits. Do not leave it missing.
- For confirm_revisit_node/request_merge_nodes memory_ops, set candidate_ref to one value from memory.candidate_refs.revisits.
...
- If memory.place_recognition.revisit_candidates is non-empty, include a memory_ops judgement whenever you are confident enough and spatial_plausibility.accepted=true; otherwise reject/request_observation.
