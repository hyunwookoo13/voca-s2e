...
class NavAgentConfig:
    ...
    # Spatial plausibility gate for VLM-confirmed revisits.  These thresholds
    # are intentionally configurable because indoor rooms, long corridors, and
    # odometry drift have different acceptable parallax/baseline ranges.
    revisit_max_same_place_baseline_m: float = 3.0
    revisit_extended_same_place_baseline_m: float = 5.0
    revisit_max_same_place_hops: int = 12
    force_front_view_waypoint: bool = True
    negative_branch_guard_deg: float = 45.0
    mark_deadlock_suspected_on_scan: bool = True

    # GaP-lite safety sidecar.  Candidate refs are stricter than raw node/edge
    # ids because they force Qwen to choose only candidates proposed in the
    # current backend snapshot.
    gap_lite_enabled: bool = True
...
class EpisodeResult:
    ...
    def summary(self) -> Dict[str, Any]:
        return {
            ...
            "validation_failures": sum(1 for s in self.step_results for cp in s.validation_checkpoints if cp.get("validate") and not cp.get("passed") and cp.get("severity") == "error"),
        }
...
class NavMemoryAgent:
    ...
    def __init__(
        ...
        *,
        ...
        self.last_validation_feedback: Optional[Dict[str, Any]] = None
        self.last_validation_checkpoints: List[Dict[str, Any]] = []
            ...
            if name in {"mark_deadlock", "mark_incoming_edge", "mark_current_as_deadlock_region"} and self.memory.current_node_id:
                ...
                if candidate:
                    self.memory.commit_revisit(
                        ...
                        max_same_place_baseline_m=self.config.revisit_max_same_place_baseline_m,
                        extended_same_place_baseline_m=self.config.revisit_extended_same_place_baseline_m,
                        max_same_place_hops=self.config.revisit_max_same_place_hops,
                    )
                ...
                if keep and remove:
                    ...
                    if verification.accepted:
                        self.memory.soft_merge_revisit(
                            str(keep),
                            str(remove),
                            frame_index=frame_index,
                            reason=str(op.get("reason") or "vlm_requested_soft_merge"),
                            vlm_confidence=conf,
                        )
    ...
    def step(
        ...
        *,
        ...
        try:
            ...
            stage = memory_context.get("policy_harness_state", {}).get("current_stage", "normal_goal_seek")
            memory_context["policy_harness_state"] = build_policy_harness_state(
                current_stage=str(stage),
                last_validation_feedback=self.last_validation_feedback,
                require_candidate_ref_for_go=self.config.require_candidate_ref_for_go,
                require_candidate_ref_for_revisit_ops=self.config.require_candidate_ref_for_revisit_ops,
            )
            ...
            validation_feedback: Optional[Dict[str, Any]] = None
            if self.config.gap_lite_enabled:
                vlm_output, validation_checkpoints, validation_feedback, gap_warnings = apply_gap_lite_validation(
                    vlm_output,
                    memory_context,
                    goal_bearing_deg=goal.relative_bearing_deg,
                    force_front_view_waypoint=self.config.force_front_view_waypoint,
                ...
                self.last_validation_checkpoints = list(validation_checkpoints)
                self.last_validation_feedback = validation_feedback
            ...
            if guard_warnings:
                guard_cp = make_checkpoint(
                    rule_id="NAV_GO_006",
                    name="backend_negative_branch_guard",
                    stage="action_validation",
                    passed=False,
                    message="backend blocked a go action that overlapped a known negative branch",
                    details={"warnings": guard_warnings},
                    on_fail="request_observation.directed_sweep",
                )
                validation_checkpoints = list(vlm_output.get("validation_checkpoints", validation_checkpoints)) + [guard_cp]
                validation_feedback = summarize_checkpoints(validation_checkpoints)
                vlm_output["validation_checkpoints"] = validation_checkpoints
                vlm_output["validation_feedback"] = validation_feedback
                self.last_validation_checkpoints = list(validation_checkpoints)
                self.last_validation_feedback = validation_feedback
            ...
            result = StepResult(
                ...
                validation_checkpoints=list(vlm_output.get("validation_checkpoints", validation_checkpoints)),
                validation_feedback=vlm_output.get("validation_feedback", validation_feedback),
            )
            ...
            result = StepResult(
                ...
                outcome=None,
                vlm_input=None,
                validation_checkpoints=list(self.last_validation_checkpoints),
                validation_feedback=self.last_validation_feedback,
                error=err,
            )
            self.step_logs.append(result)
            return result
    ...
    def save_run(self, out_dir: str | Path) -> None:
        ...
        for s in self.step_logs:
            logs.append({
                ...
                "validation_feedback": s.validation_feedback,
        ...
        feedback_lines = [
            "# NavMemory Feedback.md",
            "",
            "## Episode Summary",
            f"steps: {len(logs)}",
            f"num_nodes: {len([n for n in self.memory.nodes.values() if n.lifecycle.get('storage_tier') != 'archived'])}",
            f"num_edges: {len(self.memory.edges)}",
            f"num_deadlock_edges: {sum(1 for e in self.memory.edges.values() if e.is_negative())}",
            "",
            "## Validation Feedback",
        ]
        rule_counts: Dict[str, Dict[str, int]] = {}
        for s in self.step_logs:
            for cp in s.validation_checkpoints:
                rid = str(cp.get("rule_id"))
                entry = rule_counts.setdefault(rid, {"passed": 0, "failed": 0})
                if cp.get("passed"):
                    entry["passed"] += 1
                else:
                    entry["failed"] += 1
        if rule_counts:
            for rid in sorted(rule_counts):
                c = rule_counts[rid]
                feedback_lines.append(f"- {rid}: passed={c['passed']} failed={c['failed']}")
        else:
            feedback_lines.append("- no validation checkpoints recorded")
        feedback_lines.extend([
            "",
            "## Safety Policy",
            "- Prefer duplicate nodes over false-positive same-place merge.",
            "- Unknown or avoid=true candidate_ref is recovered with request_observation.",
            "- Pose graph optimization remains TODO and is not executed.",
            "",
        ])
