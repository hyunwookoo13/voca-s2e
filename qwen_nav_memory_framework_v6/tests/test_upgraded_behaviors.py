...
class LeftGoVLM(BaseVLMClient):
    def decide(self, vlm_input):
        ...
        ref = None
        for cand in vlm_input.get("memory", {}).get("candidate_refs", {}).get("exits", []):
            if cand.get("view_type_hint") == "left" and not cand.get("avoid"):
...
class FrontGoVLM(BaseVLMClient):
    def decide(self, vlm_input):
        ...
        ref = None
        for cand in vlm_input.get("memory", {}).get("candidate_refs", {}).get("exits", []):
            if cand.get("view_type_hint") == "front" and not cand.get("avoid"):
                ref = cand.get("candidate_ref")
                break
        return make_go_output(
...
class UpgradedBehaviorTest(unittest.TestCase):
    def test_directional_failure_creates_virtual_negative_edge_without_relocalizing(self):
        graph = MemoryGraph(embedding_dim=4)
        src = graph.add_node(
            frame_index=0,
            image_ref=None,
            embedding=None,
            semantic_summary="current place",
        )

        first = graph.record_directional_failure(
            src,
            RelativePose2D(dx_m=0.75, dy_m=0.0, dyaw_deg=0.0),
            frame_index=1,
            reason="no_progress",
            confirmed=False,
            evidence={"selected_candidate_ref": "px_v00_r0_c0"},
        )

        self.assertEqual(graph.current_node_id, src)
        self.assertEqual(len(graph.nodes), 2)
        self.assertEqual(len(graph.edges), 1)
        edge = graph.edges[first["edge_id"]]
        failed_node = graph.nodes[first["failed_frontier_node_id"]]
        self.assertEqual(edge.relation_type, "directional_failure")
        self.assertEqual(edge.traversal["status"], "deadlock_entry_candidate")
        self.assertTrue(edge.is_negative())
        self.assertEqual(failed_node.node_type, "failed_frontier")
        self.assertEqual(failed_node.negative_memory["avoid_scope"], "directional_edge_only")

        second = graph.record_directional_failure(
            src,
            RelativePose2D(dx_m=0.8, dy_m=0.1, dyaw_deg=0.0),
            frame_index=2,
            reason="repeated_no_progress",
            confirmed=True,
        )

        self.assertEqual(second["edge_id"], first["edge_id"])
        self.assertTrue(second["reused_existing_edge"])
        self.assertEqual(len(graph.nodes), 2)
        self.assertEqual(graph.edges[first["edge_id"]].traversal["status"], "blocked")
        context = graph.build_vlm_memory_context(
        ...
        failed_candidates = [
            ...
            if candidate.get("edge_id") == first["edge_id"]
        ]
        self.assertEqual(len(failed_candidates), 1)
        self.assertTrue(failed_candidates[0]["avoid"])
        self.assertEqual(context["graph_summary"]["num_place_nodes"], 1)
        self.assertEqual(context["graph_summary"]["num_failed_frontier_nodes"], 1)
    ...
    def test_context_exposes_place_recognition_candidates(self):
        ...
        self.assertEqual(ctx["schema_version"], "nav_memory_context_v6")
        self.assertIn("candidate_refs", ctx)
        self.assertIn("nav_skill_cards", ctx)
