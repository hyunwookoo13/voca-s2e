...
class VLMDecisionPromptTest(unittest.TestCase):
    def test_prompt_describes_decision_schema_and_navigation_constraints(self):
        ...
        self.assertIn("tracking_loss", prompt.system)
        self.assertIn("choose LOOK_AROUND", prompt.system)
        self.assertIn("blocked", prompt.system)
        self.assertIn("choose RESELECT_GOAL", prompt.system)
        self.assertIn("copy its goal_xy", prompt.system)
