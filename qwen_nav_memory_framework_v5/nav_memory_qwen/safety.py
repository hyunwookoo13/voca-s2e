...
def _view_type_for_id(view_id: Any, available_views: List[Dict[str, Any]]) -> Optional[str]:
    for view in available_views:
        if view.get("view_id") == view_id:
            value = view.get("view_type")
            return str(value) if value is not None else None
    return None


def _view_id_for_type(view_type: Any, available_views: List[Dict[str, Any]]) -> Optional[int]:
    for view in available_views:
        if view.get("view_type") == view_type:
            value = view.get("view_id")
            try:
                return int(value)
            except Exception:
                return None
...
def _reasoning_value(output: Dict[str, Any], primary_key: str, alternate_key: str, *, fallback: str) -> str:
    reasoning = output.get("reasoning") if isinstance(output.get("reasoning"), dict) else {}
    value = reasoning.get(primary_key)
    if value is None:
        value = reasoning.get(alternate_key)
    if value is None:
        value = fallback
    return str(value)
...
def sanitize_vlm_output(output: Dict[str, Any], vlm_input: Dict[str, Any]) -> Tuple[Dict[str, Any], List[str]]:
    ...
    if action == "go":
        ...
        point = output.get("selected_image_point") or fine_goal.get("point_px") or fine_goal.get("selected_image_point")
        view_type = output.get("selected_view_type") or fine_goal.get("view_type") or fine_goal.get("selected_view_type")
        view_id = output.get("selected_view_id")
        if view_id is None:
            view_id = fine_goal.get("view_id")
        if view_id is None:
            view_id = fine_goal.get("selected_view_id")
        if view_type is None and view_id is not None:
            view_type = _view_type_for_id(view_id, views)
        if view_id is None and view_type is not None:
            view_id = _view_id_for_type(view_type, views)
        ...
        safe = make_go_output(
            ...
            width=width,
            height=height,
            decision_reason=_reasoning_value(output, "decision_reason", "reason_code", fallback="G02_VISIBLE_FLOOR_TOWARD_GOAL"),
            goal_reason=_reasoning_value(output, "goal_reason", "reason_code", fallback="F02_VISIBLE_FLOOR_TOWARD_GOAL"),
            short_text=_reasoning_value(output, "short_text", "reason", fallback="sanitized go output"),
