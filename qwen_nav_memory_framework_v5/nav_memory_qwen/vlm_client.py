...
class OpenAICompatibleVLMClient(BaseVLMClient):
    ...
    def from_env(cls) -> "OpenAICompatibleVLMClient":
        ...
        timeout_s = float(os.getenv("QWEN_TIMEOUT_S", "300"))
        max_tokens = int(os.getenv("QWEN_MAX_TOKENS", "4096"))
        max_json_retries = int(os.getenv("QWEN_MAX_JSON_RETRIES", "1"))
        image_max_side = int(os.getenv("QWEN_IMAGE_MAX_SIDE", "1024"))
        jpeg_quality = int(os.getenv("QWEN_JPEG_QUALITY", "85"))
        extra_payload = cls._extra_payload_from_env()
        return cls(
            base_url=base_url,
            api_key=api_key,
            model=model,
    ...
    def _extra_payload_from_env() -> Dict[str, Any]:
        raw = os.getenv("QWEN_EXTRA_PAYLOAD_JSON", "").strip()
        if not raw:
            return {}
        data = json.loads(raw)
    ...
    def _build_user_content(self, vlm_input: Dict[str, Any], retry_instruction: Optional[str] = None) -> List[Dict[str, Any]]:
    ...
    def _build_payload(self, vlm_input: Dict[str, Any], retry_instruction: Optional[str] = None) -> Dict[str, Any]:
        payload = {
            "model": self.model,
            "messages": [
                {"role": "system", "content": SYSTEM_PROMPT},
                {"role": "user", "content": self._build_user_content(vlm_input, retry_instruction=retry_instruction)},
            ],
    ...
    @classmethod
    def _response_text_candidates(cls, data: Dict[str, Any]) -> List[str]:
        candidates: List[str] = []
        ...
        if choices:
            ...
            if isinstance(message, dict):
                for key in ("content", "reasoning_content", "reasoning"):
                    text = cls._stringify_text_value(message.get(key))
                    if text:
                        candidates.append(text)
            text = cls._stringify_text_value(first.get("text")) if isinstance(first, dict) else None
            if text:
                candidates.append(text)
        top_level_text = cls._stringify_text_value(data.get("output_text"))
        if top_level_text:
    ...
    def decide(self, vlm_input: Dict[str, Any]) -> Dict[str, Any]:
        ...
        last_text = ""
        attempts = max(0, int(self.max_json_retries)) + 1
        for attempt in range(attempts):
            retry_instruction = None
            if attempt > 0:
                retry_instruction = (
                    "The previous model response did not contain a parseable JSON object. "
                    "Return only one valid JSON object matching schema_version nav_vlm_waypoint_v1. "
                    "Do not include prose, markdown fences, or hidden reasoning."
                )
            data = self._post_chat_completion(self._build_payload(vlm_input, retry_instruction=retry_instruction))
            candidates = self._response_text_candidates(data)
        ...
        if last_error is not None:
            raise ValueError(f"failed to extract VLM JSON after {attempts} attempt(s): {last_error}; last_text={last_text[:240]!r}") from last_error
        raise ValueError(f"failed to extract VLM JSON after {attempts} attempt(s): response had no text candidates")
