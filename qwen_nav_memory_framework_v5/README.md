# qwen-nav-memory-framework v0.5
...
VLM-grounded place recognition with soft merge
  ...
  revisit nodes are preserved by default; canonical and revisit nodes are connected by same-place relative edges
  image paths/descriptions are shared through same_place_clusters instead of deleting the revisit baseline

Spatial plausibility gate for revisit / merge
  VLM visual recognition alone is not enough
  backend composes T_candidate_to_current through the relative-pose graph
  merge is accepted only if the baseline is physically plausible for same-place parallax
  place-category gates are conservative: corridor/doorway/unknown nodes use tighter thresholds than intersections or rooms
  far visual aliases are rejected; same-region hints may be exposed, but same-place merge is not committed
  false positive merge prevention has priority over eliminating duplicate nodes
...
The OpenAI-compatible client extracts JSON from `message.content`, Qwen-style `reasoning_content`, and plain `text` fallbacks. If the first response does not contain a parseable `nav_vlm_waypoint_v1` object, it retries with a JSON-only repair instruction before failing.
...
The top-level input remains `nav_vlm_waypoint_v1`, but `memory` is upgraded to `nav_memory_context_v5`:
...
```json
{
  ...
  "same_place_clusters": [
    {
      "canonical_node_id": "n_00001",
      "member_node_ids": ["n_00001", "n_00008"],
...
request_merge_nodes / merge_nodes  # soft-link by default; hard merge remains a backend-only function
mark_incoming_edge
mark_deadlock
mark_blocked_edge
mark_escape_edge
