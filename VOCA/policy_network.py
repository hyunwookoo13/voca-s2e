...
class PixelNavPolicy(nn.Module):
    ...
    def forward(self,goal_mask,goal_image,episode_image):
        ...
        sequence_length = epc_token.shape[1]
        if sequence_length > self.max_token_length:
            raise ValueError(
                "episode sequence length {} exceeds configured maximum {}".format(
                    sequence_length,
                    self.max_token_length,
                )
            )
        pos_indice = torch.arange(sequence_length).expand(epc_token.shape[0],sequence_length).to(self.device)
        pos_embed = self.po_embedding(pos_indice)
        epc_token = epc_token + pos_embed
        tgt_mask = generate_square_subsequent_mask(sequence_length).to(self.device)
